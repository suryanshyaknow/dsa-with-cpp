...
// head_ref is the reference of the head i.e. points to the head.
// *head_ref contains the address of the head.
// **head_ref is the pointer that points to the address of the pointer head_ref.
{
    // if(head_ref->next == NULL)
    // {
    //     cout << "Can't push cuz Linkedlist ain't empty!" << endl;
    //     return;
    // }
    Node *new_node = new Node();  // dynamically created a node using pointer.
    new_node->data = new_val;     // (*new_node).data
    new_node->next = (*head_ref); // (*new_node).next // next is itself a pointer and now it's literally storing the address of head.
...
void insertAt(Node *prev_node, int new_val)
{
    if (prev_node == NULL)
    {
        cout << "Previous Node seems to NULL!";
        return;
    }
    // deafult Step: Create a node to be inserted!
    Node *new_node = new Node();
    new_node->data = new_val;

    // Step 1: Make it point to the node that's gotta be dragged and make space for new_node
...
int main()
{
    ...
    // so its obvious pointer of pointer will be do the needful
